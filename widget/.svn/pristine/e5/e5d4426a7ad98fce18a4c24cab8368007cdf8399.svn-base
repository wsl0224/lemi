...
//			$api.css($api.dom('.img'), 'background-image:url(' + api.pageParam.avatar + ')');
			...
			//获取观众手机ip
			//api.require('ipAddress').getIp(function(ret, err) { 去除上传IP
				...
				ajax.get({
					...
					success: function(ct) {
						...
						QJ.Core.authRoomSession({  //阿里云自建改亲加加回去大斌
							roomId: self.session.roomId, //阿里云自建改亲加加回去大斌
						password: self.session.password, //阿里云自建改亲加加回去大斌
							nickname: self.session.nickname, //阿里云自建改亲加加回去大斌
							success: function() { //阿里云自建改亲加加回去大斌
								//查询直播间详情, 获取在线看直播的人数，直播状态
								QJ.Core.getLiveContext({ //阿里云自建改亲加加回去大斌
									roomId: self.session.roomId, //阿里云自建改亲加加回去大斌
									password: self.session.password,  //阿里云自建改亲加加回去大斌
									nickname: self.session.nickname, //阿里云自建改亲加加回去大斌
...
//			});
		...
		getRoomMessage: function() {
			...
			Chat.sendText(ret.value.level+'级升到'+ret.value.newlevel+'级,加油哦', 0);
			memberinfo = $api.getStorage(CONFIG.memberInfo);
			memberinfo.level = ret.value.newlevel;
