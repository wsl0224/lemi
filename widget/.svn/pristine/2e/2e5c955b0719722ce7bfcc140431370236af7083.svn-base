...
//				Debug.alt(messageId)
		...
		render: function(args){
			...
			if(args.type == 'new'){
			//alert('args.message.objectName'+args.message.objectName);
				switch(args.message.objectName){
					...
					case 'RC:CmdMsg'://命令信息
						...
						break;	
						case 'RC:LBSMsg'://图片类型
						if(args.message && args.message.content && JSON.parse(args.message.content.extra).type == '1211'){
							T.append(args.container, template[6], args.message);
						}else{
							T.append(args.container, template[2], args.message);
						}
						break;
				...
				if(args.message instanceof Array && args.message.length > 0){
					...
					//alert('value.objectName'+value.objectName);
						switch(value.objectName){
							...
							case 'RC:CmdMsg'://命令信息
								...
								break;
									case 'RC:LBSMsg'://位置类型
									//alert('弹了个位置'+JSON.parse(value.content.extra).type);
									//alert('1弹了个位置'+args.containere);
									//alert('2弹了个位置'+JSON.stringify(value));
									//alert('3弹了个位置'+JSON.stringify(template));
								if(value.extra && JSON.parse(value.extra).type == 'recallMsg') {
								
									T.prepend(args.container, template[3], value);
								}else if(value.content && value.content.extra && JSON.parse(value.content.extra).type == '1211'){
									
									T.prepend(args.container, template[6], value);
								}else {
