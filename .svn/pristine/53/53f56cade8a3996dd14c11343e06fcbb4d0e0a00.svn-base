...
//					alert(11111);
		...
		live: function() {
			...
			if(!this.cameraReady) {
				Tool.toast('正在打开摄像头,请稍侯');
				 //setTimeout("live()","2000"); //没打开摄像头，过2秒继续打开
				return;
			}
			// if (!name) {
			  // Tool.toast('11给直播写个标题吧~');
			  // return;
			// }
			...
			QJ.Publisher.login(function() {  ////如果是阿里云推流则注释掉这一正行，因为模式2不需要登录房间先
				...
				//$api.addCls($api.dom('.start'), 'hidden'); //隐藏开始直播
				//$api.addCls($api.dom('.close'), 'hidden'); //隐藏开始直播
